...
#### 4-2-2. Avro
1. row 기반의 바이너리 storage 포맷이다. 데이터의 스키마의 정의는 json 으로 되어있다.
2. 데이터 serialization capacity 가 좋다.
3. schema evolution 이 가능하고, 유연하다.

#### 4-2-3. Parquet
1. big-data에 이상적인 columnar storage 이다.
  a. 데이터 필터링(map), aggregation(reduce) 의 작업에서 빠르다.
  b. 압축 효율이 좋다.
2. 복잡한 nested data 구조를 표현할 수 있는 columnar 포맷이다.

#### 4-2-4. ORC
1. Optimized Row Columnar (ORC) file format을 지향하고, 기본적으로 columnar의 특징을 갖는다.
2. 파일 형식 자체에 index에 정보가 있어 성능이 좋다.
3. 인덱스 때문에 스키마의 순서가 중요하다.
